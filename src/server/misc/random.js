let Random = function (a) {
	...
	this.mt[0] = a >>> 0; for (this.mti = 1; this.mti < this.N; this.mti++)a = this.mt[this.mti - 1] ^ this.mt[this.mti - 1] >>> 30, this.mt[this.mti] = (((a & 4294901760) >>> 16) * 1812433253 << 16) + (a & 65535) * 1812433253 + this.mti, this.mt[this.mti] >>>= 0;
};
...
c[a & 1]; this.mti = 0;
	...
	if (this.pythonCompatibility) this.skip && this.genrand_int32(), this.skip = !0; return this.genrand_int32() * (1 / 4294967296);
};
Random.prototype.genrand_real3 = function () {
	return (this.genrand_int32() + 0.5) * (1 / 4294967296);
}; Random.prototype.genrand_res53 = function () {
	let a = this.genrand_int32() >>> 5, c = this.genrand_int32() >>> 6; return (a * 67108864 + c) * 1.1102230246251565E-16;
}; Random.prototype.LOG4 = Math.log(4); Random.prototype.SG_MAGICCONST = 1 + Math.log(4.5); Random.prototype.exponential = function (a) {
	let c = this.random(); return -Math.log(c) / a;
};
...
Random.prototype.triangular = function (a, c, b) {
	let f = (b - a) / (c - a), e = this.random(); return e <= f ? a + Math.sqrt(e * (c - a) * (b - a)) : c - Math.sqrt((1 - e) * (c - a) * (c - b));
}; Random.prototype.uniform = function (a, c) {
	return a + this.random() * (c - a);
}; Random.prototype.weibull = function (a, c) {
	let b = 1 - this.random(); return a * Math.pow(-Math.log(b), 1 / c);
};
...
extend(true, random, {
	norm: function (low, high) {
		...
		let roll = this.normal(0, 0.35);
		if (roll < -1)
			roll = -1;
		else if (roll > 1)
			roll = 1;

		let result = (mid + (roll * range));
		return result;
	},

	expNorm: function (low, high) {
		let roll = random.norm(0, 100);
		if (roll > 50)
			roll = 100 - roll;
